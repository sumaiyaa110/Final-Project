...
export default function CustomersPage() {
  ...
  const filteredCustomers = useMemo(() => {
    ...
    return customers.filter((customer) => {
      ...
      const matchRisk =
        riskFilter === "All Risk Levels" || customer.risk === riskFilter;

      const matchStatus =
        statusFilter === "All Statuses" || customer.status === statusFilter;

      const matchDevice =
        deviceFilter === "" || customer.devices.includes(deviceFilter);

      return matchSearch && matchRisk && matchStatus && matchDevice;
    });
  }, [customers, search, riskFilter, statusFilter, deviceFilter]);
    ...
    <AdminLayout>
      <div className="customers-page">
        ...
        {deviceFilter && (
          <div className="customers-device-filter-banner">
            <div className="customers-device-filter-info">
              <span>Showing customers using device</span>

              <strong>{deviceFilter}</strong>
            </div>

            <button onClick={clearDeviceFilter}>Clear Device Filter ×</button>
          </div>
        )}
        ...
        <div className="customers-filter-card">
          ...
          <button className="customers-reset-button" onClick={resetFilters}>
            Reset
          </button>
        </div>
        ...
        <div className="customers-table-card">
          <div className="customers-table-header">
            <div>
              ...
              <p>Registered customers and their current risk activity.</p>
            </div>
                ...
                {loading ? (
                  ...
                  filteredCustomers.map((customer) => (
                    <tr key={customer.id}>
                      ...
                      <td>
                        <span
                          className={`customer-risk-badge risk-${customer.risk.toLowerCase()}`}
                        >
                          {customer.risk}
                        </span>
                      </td>
                      ...
                      <td>
                        <span
                          className={`customer-status-badge status-${customer.status.toLowerCase()}`}
                        >
